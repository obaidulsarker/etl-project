...
def main():
    ...
    ###########################################################
    # CHECKING FOR FILES WHICH WERE RESUBMITTED WITHOUT FIXES #
    ###########################################################

    for path_, subdirs, files_ in os.walk(excel_dir):
        for file_ in files_:
            if (file_.endswith('_ERROR.xlsx') or file_.endswith('_CORRUPT.xls') or file_.endswith('_RESUBMISSION_ERROR.xlsx') or \
               file_.endswith('_INVALID_ERROR.xlsx') or file_.endswith('_DUPLICATE_COLUMN_ERROR.xlsx')) :
                temp_=os.path.join(path_, file_)
                shutil.move(temp_, invalid_path) #Move file to Invalid File to Folder
                nnname=temp_.split("/")[-1]
                file_name_list.append(nnname)
                file_path_list.append(temp_)
                valid_invalid_list.append('INVALID_RESUBMISSION')
                current_directory.append(f"{invalid_path}{nnname}")
    ...
    print('PRE-VALIDATION (Phase-I) IN PROGRESS...')
    ...
    print('PRE-VALIDATION (Phase-I) COMPLETED!')
    print('PRE-VALIDATION (Phase-II) IN PROGRESS...')
    ###########################################################################
    # NEW VALIDATION ALGORITHM, THAT WORKS ON O/P OF OLD VALIDATION ALGORITHM #
    ###########################################################################

    partner_reg_conn = psycopg2.connect(database=dbname_reg, user=dbuser,\
                                    password=dbpassword, host=dbhost, port=dbport)
    
    # Extracting the required data from tables below:

    # schemes_scheme table: [scheme_id]
    schemes_scheme=pd.read_sql(f"select * from schemes_scheme", \
                            con=partner_reg_conn)
    scheme_id_list=schemes_scheme['scheme_id'].to_list()

    # schemes_corporatetrustee table: [trustee_id]
    schemes_corporatetrustee=pd.read_sql(f"select * from schemes_corporatetrustee", \
                                con=partner_reg_conn)
    trustee_id_list=schemes_corporatetrustee['trustee_id'].to_list()

    # schemes_pensionfundcustodian table: [fund_custodian_id]
    schemes_pensionfundcustodian=pd.read_sql(f"select * from schemes_pensionfundcustodian", \
                                    con=partner_reg_conn)
    fund_custodian_id_list=schemes_pensionfundcustodian['fund_custodian_id'].to_list()

    # schemes_fundmanager table: [fund_manager_id]
    schemes_fundmanager=pd.read_sql(f"select * from schemes_fundmanager", \
                                    con=partner_reg_conn)
    fund_manager_id_list=schemes_fundmanager['fund_manager_id'].to_list()

    # Get a list of VALID files from previous algorithm:
    valid_excel_with_path=[]
    for path_, subdirs, files_ in os.walk(valid_path):
        for file_ in files_:
            if (file_.endswith('.xlsx') or file_.endswith('.XLSX')) :
                f_p=os.path.join(path_, file_)
                valid_excel_with_path.append(f_p)

    # Extract list of worksheets from each VALID File:
    list_of_worksheets_=[] # creating an empty list to store list of lists of workbook names
    for excel in valid_excel_with_path:
        try:
            wb = openpyxl.load_workbook(excel)
            list_of_worksheets_.append(wb.sheetnames)
        except:
            list_of_worksheets_.append('Invalid')
            print('INVALID EXCEL FILE')

    # Filter the worksheet list with only "Dataset_" sheets:
    filtered_worksheet_=[] # creating an empty list to store list of lists of filtered worksheet names
    for i in list_of_worksheets_:
        dataset_list = [item for item in i if item.startswith("Dataset_")]
        filtered_worksheet_.append(dataset_list)

    # NEW VALIDATION ALGORITHM:
    for index,i in enumerate(valid_excel_with_path):
        valid_sheet_listo=[]
        for sheet_name in filtered_worksheet_[index]:
            df_=pd.read_excel(i,sheet_name=sheet_name)
            df_=df_.dropna(how='all') # Drop Empty Rows
            df_.columns=df_.columns.str.strip() # Remove Whitespace from Column Names
            df_.columns=df_.columns.str.lower() # Changing all column names to lower case
            if len(df_['entity_id'].unique())==1:
                if list(df_['entity_id'].unique())[0] in scheme_id_list:
                    valid_sheet_listo.append(sheet_name)
                    #print(df_['entity_id'].unique(),'schemes_scheme')
                    
                elif list(df_['entity_id'].unique())[0] in trustee_id_list:
                    df_is_active_CTA=\
                    pd.read_sql(f"select schemes_corporatetrusteeappointment.is_active, \
                    schemes_corporatetrustee.trustee_id  \
                    from schemes_corporatetrusteeappointment,schemes_corporatetrustee WHERE \
                    schemes_corporatetrustee.id=schemes_corporatetrusteeappointment.trustee_id", con=partner_reg_conn)
                    df_is_active_CTA=df_is_active_CTA[df_is_active_CTA['is_active']==True]
                    list_1=df_is_active_CTA['trustee_id'].to_list()
                    if df_['entity_id'].unique() in list_1:
                        #print(df_['entity_id'].unique(),'schemes_corporatetrustee is ACTIVE')
                        valid_sheet_listo.append(sheet_name)
                    else:
                        print(df_['entity_id'].unique(),sheet_name,'INVALID')
                        
                elif list(df_['entity_id'].unique())[0] in fund_custodian_id_list:
                    df_is_active_PFA=\
                    pd.read_sql(f"select schemes_pensionfundcustodianappointment.is_active, \
                    schemes_pensionfundcustodian.fund_custodian_id  \
                    from schemes_pensionfundcustodianappointment,schemes_pensionfundcustodian WHERE \
                    schemes_pensionfundcustodian.id=schemes_pensionfundcustodianappointment.fund_custodian_id",\
                                con=partner_reg_conn)
                    df_is_active_PFA=df_is_active_PFA[df_is_active_PFA['is_active']==True]
                    list_2=df_is_active_PFA['fund_custodian_id'].to_list()
                    if df_['entity_id'].unique() in list_2:
                        #print(df_['entity_id'].unique(),'schemes_pensionfundcustodian is ACTIVE')
                        valid_sheet_listo.append(sheet_name)
                    else:
                        print(df_['entity_id'].unique(),sheet_name,'INVALID')
                    
                elif list(df_['entity_id'].unique())[0] in fund_manager_id_list:
                    df_is_active_FMA=\
                    pd.read_sql(f"select schemes_fundmanagerappointment.is_active, \
                    schemes_fundmanager.fund_manager_id  \
                    from schemes_fundmanagerappointment,schemes_fundmanager WHERE \
                    schemes_fundmanager.id=schemes_fundmanagerappointment.fund_manager_id", con=partner_reg_conn)
                    df_is_active_FMA=df_is_active_FMA[df_is_active_FMA['is_active']==True]
                    list_3=df_is_active_FMA['fund_manager_id'].to_list()
                    if df_['entity_id'].unique() in list_3:
                        #print(df_['entity_id'].unique(),'schemes_fundmanager is ACTIVE')
                        valid_sheet_listo.append(sheet_name)
                    else:
                        print(df_['entity_id'].unique(),sheet_name,'INVALID')
                else:
                    print(f'Entity_ID doesnot Exist! {sheet_name} [A]')
                        #Adding DataFrame to SQL DB:
                        #print(important_columns)
            else:
                print(f'Entity_ID ERROR! {sheet_name} [B]')
    
        f_naam=i.split("/")[-1]
        if set(filtered_worksheet_[index]) == set(valid_sheet_listo):
            f_name_v = i.split("/")[-1]
            file_name_list.append(f_name_v)
            file_path_list.append(i)
            valid_invalid_list.append('VALID')
            print(f"File: {f_naam} is VALID!")
            current_directory.append(f"{valid_path}{f_name_v}")
            datetime_list.append(datetime.datetime.now())
        else:
            missing__=(set(filtered_worksheet_[index])) - (set(valid_sheet_listo))
            shutil.move(i, invalid_path) #Move Invalid File to Folder from Main Directory
            f_name_split__=f_naam.split('.') #split filename from the extension
            new_invalid_f_name_with_path__=f"{invalid_path}/{f_name_split__[0]}_ENTITY_ERROR.{f_name_split__[-1]}"
            os.rename(f'{invalid_path}/{f_naam}', new_invalid_f_name_with_path__) #Renaming File name.
            print(f"File: {f_naam} has INVALID!")
            f_name_v = i.split("/")[-1]
            file_name_list.append(f_name_v)
            file_path_list.append(i)
            valid_invalid_list.append('INVALID')
            current_directory.append(f"{new_invalid_f_name_with_path__}")
            datetime_list.append(datetime.datetime.now())
            
            for sheet_missing_ in missing__:
                sheet_missing_=str(sheet_missing_).replace('{','').replace('}','').replace("'",'')
                sheet_missing_=sheet_missing_.strip()
                print('Missing:',sheet_missing_)
                file_loc__ = f"{new_invalid_f_name_with_path__}"
                df_excel__=pd.read_excel(file_loc__,engine='openpyxl',sheet_name=sheet_missing_)
                df_excel__=df_excel__.dropna(how='all') # Drop Empty Rows
                df_excel__.columns=df_excel__.columns.str.strip() # Remove Whitespace from Column Names
                df_excel__.columns=df_excel__.columns.str.lower() # Changing all column names to lower case
                
                error_code_lst__=[]
                error_desc__=[]

                try:
                    for i in df_excel__['entity_id'].to_list():
                        if (df_excel__['entity_id'].mode()[0] in scheme_id_list or \
                            df_excel__['entity_id'].mode()[0] in trustee_id_list or \
                            df_excel__['entity_id'].mode()[0] in fund_custodian_id_list or \
                            df_excel__['entity_id'].mode()[0] in fund_manager_id_list) and i == df_excel__['entity_id'].mode()[0]:
                            error_code_lst__.append('COMMON:NE')
                            error_desc__.append('Common Entity_ID : NO_ERROR')
                        else:
                            error_code_lst__.append('UNCOMMON:E')
                            error_desc__.append('Different Entity_ID : ERROR')
                except:
                    for i in range(len(df_excel__)):
                        error_code_lst__.append('MISSING:E')
                        error_desc__.append('MISSING Entity_ID : ERROR')
            
                df_excel__['ERROR_CODE']=error_code_lst__
                df_excel__['ERROR_DESCRIPTION']=error_desc__
                write_excel(file_loc__,sheet_missing_,df_excel__)
                
                if '_ERROR' not in sheet_missing_:
                    ss_ = openpyxl.load_workbook(file_loc__)
                    ss_sheet_ = ss_[f"{sheet_missing_}"]
                    ss_sheet_.title = f"{sheet_missing_}_ERROR"
                    ss_.save(file_loc__)

    print('PRE-VALIDATION (Phase-II) COMPLETED!')
    ...
    ec_map =  { 'INVALID' : 'INV',  'VALID' : 'VLD', 'CORRUPT' : 'CRPT' , 'RESUBMISSION' : 'RESUB' , 'INVALID_RESUBMISSION' : 'INVRES'}
    ...
    print('Creation of LOG Files is COMPLETED!')
    print('Starting Upload of INVALID and RESUBMISSION Files to Sharepoint!')

    #########################################################
    # STEP-5 : UPLOADING Invalid Files to Sharepoint Folder #
    #########################################################
    ...
    def upload_files(folder, keyword=None):
        ...
        for file in file_list:
            if keyword is None or keyword == 'None' or re.search(keyword, file[0]):
                ...
                print("Uploading INVALID File: ",file[0])
    ...
    def upload_files_resubmission(folder, keyword=None):
        ...
        for file in file_list:
            if keyword is None or keyword == 'None' or re.search(keyword, file[0]):
                file_content = get_file_content(file[1])
                print("Uploading RESUBMISSION File: ",file[0])
